...
add_action('init', function() {
    add_shortcode('customize-coffee-form', function($atts = [], $content = null) {
        $content .= "<script>var atts = " . json_encode($atts) . "</script>";
        $content .= file_get_contents(dirname(__FILE__) . "/order.html");
    ...
    wp_register_script('angular_script', 
        "https://cdnjs.cloudflare.com/ajax/libs/angular.js/1.3.15/angular.js", true);
    wp_enqueue_script('angular_script');
    
    wp_register_script('coffeecontroller_script', plugins_url('indexcontroller.js', __FILE__), 
        true);
